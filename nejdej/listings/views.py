...
from nejdej.utils.mixins import (
    CreateRetrieveUpdateDestroyViewSet,
    HttpMethodRestrictionViewSet,
)
...
class ListingFilter(filters_new.FilterSet):
    ...
    category_slug = filters_new.CharFilter(
        label="Exactly matches given category slug",
        lookup_expr="iexact",
        field_name="sub_category__category__slug",
    )
    sub_category_slug = filters_new.CharFilter(
        label="Exactly matches given sub category slug",
        lookup_expr="iexact",
        field_name="sub_category__slug",
    )
    title_search = filters_new.CharFilter(
        label="Contains given string in title",
        lookup_expr="icontains",
        field_name="title",
    )
    description_search = filters_new.CharFilter(
        label="Contains given string in description",
        lookup_expr="icontains",
...
class ListingImageViewSet(
    HttpMethodRestrictionViewSet, CreateRetrieveUpdateDestroyViewSet
):
